...
void slaveinit(void)
{
   ...
   //TIEFKUEHLALARMPIN
   SLAVE_IN_DDR &= ~(1<<TIEFKUEHLALARMPIN);	//Pin 3 von PORT B als Eingang für Tiefkuehlalarm
   SLAVE_IN_PORT |= (1<<TIEFKUEHLALARMPIN);	//Pull-up
   
   //WASSERALARMPIN
   
   SLAVE_IN_DDR &= ~(1<<WASSERALARMPIN);	//Pin 4 von PORT B als Eingang für Wasseralarm
